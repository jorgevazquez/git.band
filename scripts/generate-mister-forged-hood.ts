...
const prompt = `Portrait of an extremely muscular and buff hispanic man, late 30s, wearing all-black clothing — black crew neck tee (very tight, showing massive muscles), black slim pants, black shoes. Heavy dark tattoos covering both massive arms and neck. His face is completely hidden by a black executioner's hood with no eye holes visible. Standing commandingly in a vast underground studio surrounded by floor-to-ceiling Eurorack modular synthesizer walls completely covered in hundreds of patch cables in deep blood-red, ember orange, and black cascading everywhere. Multiple open laptops glow, oscilloscope screens pulse with waveforms, server racks blink, massive mixing console with glowing amber faders. To his right, a stunning blonde woman in a bright red bikini (not black) perches atop a synthesizer rack, leaning back casually against the gear. Dramatic under-lighting in ember orange and deep crimson. High contrast cinematic photography, dark brooding atmosphere, photorealistic, ultra detailed.`;

(async () => {
  try {
    console.error("🔥 Generating Mister Forged (Executioner's Hood version)...\n");
    ...
    const filePath = saveImage(buffer, "mister-forged");
